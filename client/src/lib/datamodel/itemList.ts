...
export class FilterList<T> implements Readable<T[]> {
	...
	concat<S>(other: FilterList<S>): FilterList<T | S> {
		return new FilterList(
			derived([this.items, other.items], ([a, b]) => [...a, ...b]),
		);
	}
	...
	reduce<S>(
		reducer: (accumulator: S, currentValue: T) => S,
		initialValue: S,
	): Readable<S> {
	...
	subscribeEach<S extends Readable<any>>(
		mapFn: (value: T) => S,
		callback: (item: T, value: any) => void,
	): Unsubscriber {
		...
		const unsubscribeFromItems = this.items.subscribe((items) => {
			...
			unsubscribes = items.map((item) =>
				mapFn(item).subscribe((value) => callback(item, value)),
			);
		});
