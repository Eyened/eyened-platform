...
export abstract class BaseItem {
	...
	async update(item: any) {
		...
		for (const key in item) {
			if (key in this) {
				if (isStateProperty(this, key)) {
					...
					console.warn(
						`property ${key} is not a state property and will not be updated`,
					);
				}
			} else {
		...
		if (!response.ok) {
			console.error(
				`failed to update ${this.endpoint} ${this.id}: ${response.statusText}`,
			);
			return;
		}
	...
	static async create(item: any) {
		...
		if (!response.ok) {
			throw new Error(
				`Failed to create ${this.endpoint}: ${response.statusText}`,
			);
		}
	...
	async delete(fromServer: boolean = true) {
		if (fromServer) {
			...
			if (!response.ok) {
				throw new Error(
					`Failed to delete ${this.endpoint} ${this.id}: ${response.statusText}`,
				);
			}
		}
