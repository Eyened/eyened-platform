...
export class Instance extends BaseItem {
	...
	get segmentations(): FilterList<Segmentation> {
		return data.segmentations.filter(
			(segmentation) => segmentation.instanceId == this.id,
		);
	}

	get modelSegmentations(): FilterList<ModelSegmentation> {
		return data.modelSegmentations.filter(
			(modelSegmentation) => modelSegmentation.instanceId == this.id,
		);
	}
}
