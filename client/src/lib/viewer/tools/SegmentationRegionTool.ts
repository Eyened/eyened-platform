...
export class SegmentationRegionTool implements Overlay {
	...
	pointerdown(e: ViewerEvent<PointerEvent>) {
		...
		this.draftBox = imageBoxFromCorners(this.startImagePos, this.startImagePos);
	}
	...
	repaint(viewerContext: ViewerContext, _renderTarget: RenderTarget) {
		...
		const p0 = viewerContext.imageToViewerCoordinates({ x: this.draftBox.x0, y: this.draftBox.y0 });
		const p1 = viewerContext.imageToViewerCoordinates({ x: this.draftBox.x1, y: this.draftBox.y1 });
