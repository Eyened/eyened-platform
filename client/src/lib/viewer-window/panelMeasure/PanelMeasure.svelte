<script lang="ts">
    ...
    const { registration } = getContext<ViewerWindowContext>(
        "viewerWindowContext",
    );

    const etdrsSchema = formSchemasByName.get(
        BUILTIN_VIEWER_FORM_SCHEMA_NAMES.ETDRS_GRID,
    )!;
...
<div id="main">
    <div class="resolution">
        <div>
            {measureTool.imageResX.toFixed(2)} × {measureTool.imageResY.toFixed(
                2,
            )} μm/pix
        </div>
        ...
        <div id="set-resolution" class:hideRes>
            <div>
                <label>
                    ...
                    <input
                        type="number"
                        bind:value={measureTool.imageResX}
                        step="0.1"
                    /> μm/pix
                </label>
            </div>
            <div>
                <label>
                    ...
                    <input
                        type="number"
                        bind:value={measureTool.imageResY}
                        step="0.1"
                    /> μm/pix
                </label>
            </div>
