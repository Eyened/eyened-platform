<script lang="ts">
	...
	const filter = (formAnnotation: FormAnnotation) => {
		...
		if (formAnnotation.instance?.laterality !== instance.laterality) return false;
		return true;
	};
	const filtered = formAnnotations.filter(filter);

	async function create() {
		createFormAnnotation(creator, instance, registrationSchema, taskContext?.subTask);
	}
	...
	<div class="available">
		<ul>
			{#each $filtered.sort((a, b) => a.id - b.id) as formAnnotation (formAnnotation.id)}
				{#if formAnnotation.value.value != undefined}
