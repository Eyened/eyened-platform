<script lang="ts">
	...
	function close() {
		viewerWindowContext.browserOverlay = false;
		...
		if (subTask) {
            ...
            console.log('updating instance ids in url', currentInstanceIds);
		}
        viewerWindowContext.setInstanceIDs(currentInstanceIds);
	}

	function updateSubTaskImageLinks(currentInstanceIds: number[]) {
		console.log('updating image links');

		const newInstanceIds = currentInstanceIds.filter((id) => !initialInstanceIds.includes(id));
		const removedInstanceIds = initialInstanceIds.filter((id) => !currentInstanceIds.includes(id));
		...
		for (const instanceid of removedInstanceIds) {
			const link = subTaskImageLinks.find(
				(link) => link.subTask == subTask && link.instanceid == instanceid
			);
			if (link) {
...
<div id="browser-overlay">
	<div class="button-container">
		{#if subTask}
			...
			<input id="updateImageLinks" type="checkbox" bind:checked={updateImageLinks} />
		{/if}
