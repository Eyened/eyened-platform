...
export const featureLabels: {
	[featureName: string]: { [layerName: string]: number };
} = {
	"Macular layers": {
		background: 0,
		NFL: 1,
...
export async function createMaskedAnnotation(
	dialogue: Writable<DialogueType>,
	...
	if (!annotationType) {
		dialogue.set(
			"No annotation type found for Segmentation 2D masked + R/G mask",
		);
		return;
	}
