...
<script lang="ts">
	...
	import { deleteAnnotation, macularLayers } from '../panelSegmentation/segmentationUtils';
	...
	function getNewAnnotation(interpretation: string) {
		...
		const annotationType = data.annotationTypes.find(
			(at) => at.name == 'Segmentation OCT B-scan' && at.interpretation == interpretation
		);
		if (!annotationType) {
...
<div class="main" class:active>
	<label>
		Opacity:
		<input type="range" bind:value={layerSegmentationOverlay.alpha} min="0" max="1" step="0.01" />
	</label>
								...
								<li>
									<label>
										{#if annotationType.interpretation == 'Layer bits'}
											<input type="checkbox" value={layer} bind:group={selectedLabelNames} />
										{:else if annotationType.interpretation == 'Label numbers'}
											<input type="radio" value={layer} bind:group={selectedLabelNames} />
										{/if}
										{layer}
									</label>
								</li>
