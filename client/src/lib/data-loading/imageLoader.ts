...
export class ImageLoader {
	...
	async load(instance: InstanceGET): Promise<LoadedImages> {
		...
		const extension = instance.dataset_identifier
			.toLowerCase()
			.split(".")
			.pop();
	...
	async loadPngSeries(
		...
		const image = meta.images.images.find(
			(img: any) => img.source_id == source_id,
		);
	...
	async loadDicom(
		...
		if (depth > 1 || meta.x00080060 === "OPT") {
			...
			return this.returnImage3D(
				new Image3D(instance, this.webgl, img_id, volume, dimensions, meta),
			);
		}
