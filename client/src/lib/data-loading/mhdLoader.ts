...
function parseMhdHeader(mhdText: string): ParsedMhdHeader {
    ...
    const spacing = toNumberArray(
        values.get("ElementSpacing") ?? values.get("ElementSize"),
    );
    ...
    const byteOrderField =
        values.get("BinaryDataByteOrderMSB") ??
        values.get("ElementByteOrderMSB");
    const byteOrderMsb =
        byteOrderField === "True" ||
        byteOrderField === "true" ||
        byteOrderField === "1";
    ...
    if (dimSize.length < 2) {
        throw new Error(
            "Invalid MHD header: DimSize must contain at least 2 values",
        );
    }
    if (!Number.isFinite(channels) || channels < 1) {
        throw new Error(
            "Invalid MHD header: ElementNumberOfChannels must be >= 1",
        );
    }
...
function convertRawToUint8(
    buffer: ArrayBuffer,
    header: ParsedMhdHeader,
    voxelCount: number,
): Uint8Array {
    ...
    if (buffer.byteLength < expectedBytes) {
        throw new Error(
            `RAW payload too small (${buffer.byteLength} < ${expectedBytes})`,
        );
    }
