<script lang="ts">
	...
	const fieldID = [...Array(10)].map(() => Math.random().toString(36)[2]).join('');
...
{#snippet query(showValue = true)}
	<div class="query">
		...
		{#if showValue}
			{#if canEdit}
				<span onclick={clear} class="selected" class:hide={schemaValidator.value === undefined}>
					{schemaValidator.value}
				</span>
			{:else}
				<span class="value" class:hide={schemaValidator.value === undefined}>
					{schemaValidator.value}
				</span>
			{/if}
		{/if}
	...
	<div class="content" class:valid={schemaValidator.isValid}>
		...
		{:else if schema.type == 'string' || schema.type == 'number' || schema.type == 'integer'}
			{#if schema.description}
				{@render query(!(schema.enum && schema.enum.length > max_radio_options))}
			{/if}
					...
					{#if inputVisible}
						<ol class:vertical>
							{#each schema.enum as option}
								<li>
									<label>
										<input type="radio" value={option} name={fieldID} bind:group={val.value} />
										<span>{option}</span>
									</label>
								</li>
							{/each}
