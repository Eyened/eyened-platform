<script lang="ts">
    ...
    let { tagType = "Annotation" }: { tagType?: TagType } = $props();
    ...
    async function handleClickStar(tagID: number) {
        if (favouriteTagIDs.has(tagID)) {
            ...
            ctx.userManager.starredTagIds =
                ctx.userManager.starredTagIds.filter((id) => id !== tagID);
        } else {
            await starTag(tagID);
            ctx.userManager.starredTagIds = [
                ...ctx.userManager.starredTagIds,
...
<div>
    ...
    {#each filtered_tags as tag}
        ...
        >
            <button
                class="inline-block"
                onclick={() => handleClickStar(tag.id)}
            >
                <Fa icon={faStar} class="inline-block" />

                <Tooltip delayDuration={50}>
                    <TooltipTrigger
                        ><span>{tag.name} ({tag.name}) </span></TooltipTrigger
                    >
                    <TooltipContent>
                        <p>
                            {tag.description
