...
export function getFdsRegistration(image: AbstractImage): PhotoLocator[] {
	...
	if (
		!instance ||
		!meta?.registration?.top_left ||
		!meta?.registration?.bottom_right ||
		!meta?.oct_shape
	) {
		return [];
	}
	const enfaceInstance = getInstanceByDataSetIdentifier(
		instance.dataset_identifier.replace(".binary", ".png"),
	);
	if (!enfaceInstance) {
		return [];
	}
	...
	const photoLocations: LinePhotoLocator[] = Array.from(
		{ length: n_scans },
		(_, i) => {
			const r = i / n_scans;
