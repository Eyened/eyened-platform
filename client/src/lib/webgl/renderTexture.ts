    ...
    readData() {
        ...
        if (
            !checkFramebufferContext(
                gl,
                this.framebuffer,
                "RenderTexture.readData",
            )
        ) {
            console.error(
                "Skipping RenderTexture.readData due to framebuffer context violation",
            );
            throw new Error(
                "Framebuffer context violation in RenderTexture.readData",
            );
        }
        gl.bindFramebuffer(gl.FRAMEBUFFER, this.framebuffer);
        let pixels;
    ...
    getData(ctx: CanvasRenderingContext2D) {
        ...
        if (
            !checkFramebufferContext(
                gl,
                this.framebuffer,
                "RenderTexture.getData",
            )
        ) {
            console.error(
                "Skipping RenderTexture.getData due to framebuffer context violation",
            );
            return;
        }
