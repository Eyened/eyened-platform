...
describe("volumeLimits", () => {
    ...
    it("accepts oversized slices for 2D array storage", () => {
        expect(
            fitsSliceStack(
                { width: 4000, height: 4000, depth: 32 },
                typicalLimits,
            ),
        ).toBe(true);
    });
    ...
    it("rejects slices that exceed 2D texture limits", () => {
        expect(
            fitsSliceStack(
                { width: 20000, height: 4000, depth: 32 },
                typicalLimits,
            ),
        ).toBe(false);
    });

    it("rejects stacks that exceed MAX_ARRAY_TEXTURE_LAYERS", () => {
        expect(
            fitsSliceStack(
                { width: 4000, height: 4000, depth: 4096 },
                typicalLimits,
            ),
        ).toBe(false);
    });
