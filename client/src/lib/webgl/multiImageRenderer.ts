...
export class MultiImageRenderer implements ImageRenderer {
    ...
    constructor(
        private readonly image: Image2D,
        private readonly registration: Registration,
    ) {
        const { webgl } = image;
        this.webgl = webgl;
        const fs_base = fs_redblue.replace(
            `// @insert mapping`,
    ...
    renderImage(viewerContext: ViewerContext, renderTarget: RenderTarget) {
        ...
        uniforms.u_size_secondary = [
            this.secondary.width,
            this.secondary.height,
        ];
    ...
    setImage(image: Image2D) {
        this.secondary = image;
        const item = this.registration.getRegistrationItem(
            this.image.image_id,
            this.secondary.image_id,
        );
