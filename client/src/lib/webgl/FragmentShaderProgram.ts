    ...
    pass(renderTarget: RenderTarget, uniforms: ShaderUniforms) {
        ...
        if (
            !checkFramebufferContext(
                gl,
                framebuffer,
                "BaseTextureShaderProgram.pass",
            )
        ) {
            console.error(
                "Skipping BaseTextureShaderProgram.pass due to framebuffer context violation",
            );
            return;
        }
    ...
    pass(renderTarget: RenderTarget, uniforms: ShaderUniforms) {
        ...
        if (
            !checkFramebufferContext(
                ...
                "PixelShaderProgram.pass",
            )
        ) {
            console.error(
                "Skipping PixelShaderProgram.pass due to framebuffer context violation",
            );
            return;
        }
    ...
    pass(renderTarget: RenderTarget, uniforms: ShaderUniforms) {
        ...
        if (
            !checkFramebufferContext(
                gl,
                renderTarget.framebuffer,
                "AffineShaderProgram.pass",
            )
        ) {
            console.error(
                "Skipping AffineShaderProgram.pass due to framebuffer context violation",
            );
            return;
        }
