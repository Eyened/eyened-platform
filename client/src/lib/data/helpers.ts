...
export async function untagInstance(instance: InstanceGET, tagId: number) {
	...
	await api.DELETE("/instances/{instance_id}/tags/{tag_id}" as any, {
		params: {
			path: { instance_id: Number(instance.id), tag_id: tagId },
		} as any,
	});
...
export async function tagFormAnnotation(
	...
	const { data } = await api.POST(
		"/form-annotations/{form_annotation_id}/tags" as any,
		{
			params: {
...
export async function untagFormAnnotation(
	...
	await api.DELETE(
		...
		{
			params: {
				path: { form_annotation_id: Number(formAnnotation.id), tag_id: tagId },
			} as any,
		},
	);
	// Update store
...
export async function tagSegmentation(seg: SegmentationGET, tagId: number) {
	const { api } = await import("../api/client");
	const { data } = await api.POST(
		"/segmentations/{segmentation_id}/tags" as any,
		{
...
export async function removeSubTaskImage(
	...
	const res = await api.DELETE(
		"/subtasks/{subtaskid}/images/{instance_id}" as any,
		{
			params: {
				path: {
					subtaskid: subtaskId,
...
): InstanceGET | undefined {
	return instances.find((inst) => inst.sop_instance_uid === SOPInstanceUid);
}
export function getInstanceByDataSetIdentifier(
	datasetIdentifier: string,
): InstanceGET | undefined {
	return instances.find(
		(inst) => inst.dataset_identifier === datasetIdentifier,
	);
}
