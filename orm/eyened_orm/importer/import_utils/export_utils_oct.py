...
from eyened_orm import ImageInstance, Segmentation, Feature
...
def get_all_etrds_grids(session, instance: ImageInstance, creatorname=None):
    query = select(Segmentation).join(ImageInstance).join(Feature).where(Feature.FeatureName == "ETDRS grid", ImageInstance.ImageInstanceID == instance.ImageInstanceID, Segmentation.Inactive == None)  # noqa: E711
...
def get_all_fovea_segmentations(session, instance: ImageInstance, creatorname=None):
    query = select(Segmentation).join(ImageInstance).join(Feature).where(Feature.FeatureName == "Fovea", ImageInstance.ImageInstanceID == instance.ImageInstanceID, Segmentation.Inactive == None)  # noqa: E711
    if creatorname is not None:
        query = query.join(creator).where(creator.CreatorName == creatorname)  # noqa: F821
    return session.execute(query).scalars().all()
