...
``Task.create_from_imagesets`` and ``SubTaskRepository.add_link``. It builds
rows generically through ``entity.model()``, so it cannot be found by grepping
for a constructor, and its entity graph (``TASK_ENTITY_SPECS``) carries no
project of its own: ``ImageInstanceID`` is a flat field on the row, with no
``ImageInstance`` entity behind it.

So the declaration is resolved here, from ``ImageInstance.ProjectID`` -- one
indexed lookup per import -- and the rule mirrors ``create_from_imagesets``:
...
* a task that **already exists** is not extended. An image from an undeclared
  ...
  changes who can see work already recorded under the narrower one.
"""
...
def declare_task_projects(session: Session, run: ImportRun) -> None:
    ...
    A no-op only when the run creates neither a link nor a task. A run that
    creates tasks must not leave any of them with an empty declaration: the
    refusal at the bottom is a different check from the unresolvable-id
    pass-over above it.
    """
    created_tasks = {
    ...
    # than refused: the link's own foreign key to ImageInstance rejects a
    # dangling id when the run is applied, which is where the importer's callers
    # catch it. Refusing here would move that failure from the write to the plan
    # and change its type from IntegrityError to ValueError.
