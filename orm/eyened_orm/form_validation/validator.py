...
def validate_all_forms(session: Optional[Session] = None, print_errors: bool = False) -> pd.DataFrame:
    ...
    for form in form_annotations:
        ...
        result = {"FormAnnotationID": form.FormAnnotationID, "SchemaName": schema_name, "IsValid": is_valid, "ErrorCount": len(errors)}
    ...
    if not results_df.empty:
        ...
        schema_stats = results_df.groupby("SchemaName").agg(TotalForms=("FormAnnotationID", "count"), ValidForms=("IsValid", "sum"))
...
def validate_all_schemas(session: Optional[Session] = None, print_errors: bool = False) -> pd.DataFrame:
    ...
    for schema in form_schemas:
        ...
        result = {"FormSchemaID": schema.FormSchemaID, "SchemaName": schema.SchemaName if schema.SchemaName else "Unknown", "IsValid": is_valid, "ErrorCount": len(errors)}
    ...
    if not results_df.empty:
        ...
        schema_stats = pd.DataFrame({"Metric": ["Total Schemas", "Valid Schemas", "Invalid Schemas", "Validity Percentage"], "Value": [total, valid, invalid, f"{(valid / total) * 100:.2f}%"]})
...
def validate_all(session: Optional[Session] = None, print_errors: bool = False) -> Dict[str, pd.DataFrame]:
    ...
    return {"schemas": schema_stats, "forms": form_stats}
