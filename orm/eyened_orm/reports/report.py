...
class Report:
    ...
    def make_summary(self):
        self.summaries = {feature_name: self.etdrs.get_summary(img, self.field_names) for feature_name, img in self.feature_images.items()}
    ...
    def generate_html_table(self):
        ...
        headers = ["Field"] + [f"{name}_{measurement}" for name in self.summaries for measurement in measurements]
        table = [headers]
        ...
        html = ["<table><thead><tr>", *[cell(header, 0) for header in table[0]], "</tr></thead><tbody>", *[map_row(row) for row in table[1:]], "</tbody></table>"]
        return "\n".join(html)
    ...
    def generate_html_report(self, image, name):
        ...
        for feature_name, summary in self.summaries.items():
            count = {k[:3]: f"{v}" for k, v in summary.items() if k.endswith("count")}
            area = {k[:3]: f"{v:.2f}" for k, v in summary.items() if k.endswith("area")}
        ...
        imgs = "".join(f"<div><h2>{feature_name}</h2>{make_img(img)}</div>" for feature_name, img in overlays.items())
