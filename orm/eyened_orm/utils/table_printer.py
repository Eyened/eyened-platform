...
class TablePrinter:
    ...
    def __init__(self, title: Optional[str] = None, theme: str = "default", compact: bool = False):
        """
    ...
    def _format_value(self, value: Any) -> str:
        ...
        if value is None:
            ...
            raw_value = value
            escaped_value = raw_value.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
            if len(raw_value) > self.max_string_length:
                display = escaped_value[: self.max_string_length - len(self.truncation_suffix)] + self.truncation_suffix
                # Build tooltip (immediate) and copy button (no <script> tag)
                tooltip_full_html = raw_value.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;").replace('"', "&quot;")
                js_full = raw_value.replace("\\", "\\\\").replace("'", "\\'").replace("\n", "\\n")
                return (
