...
class TablePrinter:
    ...
    def __init__(
        self, title: Optional[str] = None, theme: str = "default", compact: bool = False
    ):
        """
    ...
    def _format_value(self, value: Any) -> str:
        ...
        if value is None:
            ...
            if len(raw_value) > self.max_string_length:
                display = (
                    escaped_value[
                        : self.max_string_length - len(self.truncation_suffix)
                    ]
    ...
    def _repr_html_(self, data: Dict[str, Any]) -> str:
        ...
        for i, (key, value) in enumerate(data.items()):
            ...
            row_background = (
                self.row_even_background if i % 2 == 0 else self.row_odd_background
            )
...
def format_table_html(headers, rows):
    """Format tabular data as an HTML table suitable for notebook display."""
...
.eyened-table tr:nth-child(even) td {
    background-color: #fafafa;
}
.eyened-table tr:hover td {
    background-color: #eeeeee;
}
.eyened-table tr.separator td {
    border: none;
    padding: 0;
    height: 3px;
    background: black;
...
""".strip()
    ...
    def make_row(tag, cells):
        if cells is None:
            return f'<tr class="separator"><td colspan="{len(headers)}"></td></tr>'

        cell_tag = "th" if tag == "th" else "td"
        return (
            "<tr>"
            + "".join(f"<{cell_tag}>{make_cell(cell)}</{cell_tag}>" for cell in cells)
            + "</tr>"
        )

    html = [
        styles,
        '<div class="eyened-wrapper"><table class="eyened-table">',
        make_row("th", headers),
        *[make_row("td", row) for row in rows],
        "</table></div>",
    ]
    return "".join(html)
