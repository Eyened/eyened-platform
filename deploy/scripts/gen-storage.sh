...
# Which services get the dataset mounts. Every name here must be DECLARED by
# some layer COMPOSE_FILE also names, so the list is derived from COMPOSE_FILE
# rather than fixed.
#
# The condition is not tidiness — it is required, in BOTH directions.
# compose.storage.yaml is merged into whatever COMPOSE_FILE names, and a
# service that appears ONLY in the storage overlay has neither an image nor a
# build context. Measured, once per direction:
#
#   compose.workers.yaml absent, worker sections emitted anyway:
#     service "worker-cfi-roi" has neither an image nor a build context
#     specified: invalid compose project
#   compose.yaml absent (the remote GPU host runs compose.workers.yaml plus
#   this overlay and nothing else), server/fileserver emitted anyway:
#     service "server" has neither an image nor a build context specified:
#     invalid compose project
#
# So the first breaks the DEFAULT stack for everyone who never runs a worker,
# and the second breaks the remote worker host. This runs on every
# './eyened up', so changing COMPOSE_FILE and re-running is all it takes.
#
# Compose itself prefers an EXPORTED COMPOSE_FILE over the one in deploy/.env
# — env_get alone reads only the latter, so a shell export this script cannot
# see would make it derive the wrong service list while the actual project
# resolves against a different COMPOSE_FILE entirely. ${COMPOSE_FILE:-...}
# is safe under this script's own `set -eu`: it substitutes the fallback
# rather than erroring when COMPOSE_FILE is unset in the environment, and
# env_get's own `[ -f "$_file" ] || return 0` (lib.sh) already tolerates a
# missing or COMPOSE_FILE-less deploy/.env by returning empty.
_compose_file=${COMPOSE_FILE:-$(env_get COMPOSE_FILE)}
storage_services=""
case "$_compose_file" in
    *compose.yaml*)
        storage_services="server fileserver" ;;
esac
case "$_compose_file" in
    *compose.workers.yaml*)
        storage_services="$storage_services worker-cfi-roi worker-inference worker-cfi-amd worker-layersegmentation" ;;
esac

# --- compose overlay -------------------------------------------------------
if [ -s "$work/mounts" ] && [ -n "$storage_services" ]; then
    json=$(awk -F'\t' '{printf "%s\"%s\":\"%s\"", (NR > 1 ? "," : ""), $1, $2}' "$work/mounts")
    {
        echo "# GENERATED by deploy/scripts/gen-storage.sh from storage-mounts.conf."
        echo "# Do not edit: your changes will be overwritten on the next run."
        echo "services:"
        for _svc in $storage_services; do
            echo "  $_svc:"
            # EYENED_STORAGE_MOUNTS goes to every service that RESOLVES image
            # paths — the server and every worker. The fileserver only serves
            # bytes off disk through nginx and has no ORM in it, so it gets the
            # mounts and not the map.
            case "$_svc" in
                fileserver) ;;
                *) echo "    environment:"
...
elif [ -s "$work/mounts" ]; then
    # Mounts exist but COMPOSE_FILE names no layer that declares a service to
    # give them to — because it is unset and deploy/.env does not exist, or
    # exists with no COMPOSE_FILE= line at all (env_get returns empty), or it
    # is set but names neither compose.yaml nor compose.workers.yaml. Emitting
    # `services:` with nothing under it would be a null mapping and refuse the
    # whole project, so say so and emit the empty shape.
    #
    # This is a WARNING, not a failure: the state is recoverable (the next
    # './eyened up' regenerates this file) and an operator may be generating
    # before configuring COMPOSE_FILE on purpose. The comment inside the
    # generated file is not enough on its own — this file is gitignored, and
    # a remote operator who fills storage-mounts.conf before COMPOSE_FILE
    # would otherwise see only the normal success line below and a clean
    # `docker compose config`, with workers that mount nothing.
    echo "warning: COMPOSE_FILE (checked exported first, then deploy/.env) names
      neither compose.yaml nor compose.workers.yaml, so compose.storage.yaml
      has no services under it and these mounts reach nothing.
      Fix: set COMPOSE_FILE and re-run — './eyened up' does that for you." >&2
    {
        echo "# GENERATED by deploy/scripts/gen-storage.sh from storage-mounts.conf."
        echo "# COMPOSE_FILE (exported, or the COMPOSE_FILE= line in deploy/.env)"
        echo "# is unset or absent, or names neither compose.yaml nor"
        echo "# compose.workers.yaml, so no service here can receive the mounts."
        echo "# Fix: set COMPOSE_FILE and re-run — './eyened up' does that for you."
        echo "services: {}"
    } > "$work/compose.storage.yaml" ||
        die "error: could not write $work/compose.storage.yaml (see any error above)."
else
