...
_compose_file=$(env_get COMPOSE_FILE)
storage_services=""
...
if [ -s "$work/mounts" ] && [ -n "$storage_services" ]; then
    ...
    # give them to. Emitting `services:` with nothing under it would be a null
    # mapping and refuse the whole project, so say so and emit the empty shape.
    {
        echo "# GENERATED by deploy/scripts/gen-storage.sh from storage-mounts.conf."
        echo "# COMPOSE_FILE in deploy/.env names neither compose.yaml nor"
