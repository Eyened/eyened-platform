...
7. `alembic stamp --purge orm_baseline` (`--purge` is required — the squash drops the
   pre-cutover id from the revision map). Verify `alembic current` prints bare
   `orm_baseline`, with **no** `(head)`: the five revisions step 9 applies sit above it.
   Do not check for drift here — on a behind-head database `alembic check` refuses to run
   (`Target database is not up to date.`) rather than reporting anything. Step 9 is where
