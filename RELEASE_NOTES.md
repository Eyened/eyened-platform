...
7. `alembic stamp --purge orm_baseline` (`--purge` is required — the squash drops the
   ...
   `orm_baseline`, with **no** `(head)`: the five revisions step 9 applies sit above it, so
   the database is five migrations behind here and `alembic check` reports them as drift
   until step 9 runs.
