...
:::note
These steps below assume that the ORM is locally installed and configured as explained in [Getting Started](eyened_platform/getting_started).
:::
...
This example uses an environment called `prod` to determine what database to connect to and where to write thumbnails. This environment must be provided in a .env file as explained in [eyened_platform/configuration].
...
1. Update the [thumbnail generation code](https://github.com/Eyened/eyened-platform/blob/main/orm/eyened_orm/importer/thumbnails.py).
2. The ORM includes a [notebook](https://github.com/Eyened/eyened-platform/blob/main/orm/notebooks/test_thumbnail_generator.ipynb) to easily query a sample of images and visualize their generated thumbnails.
