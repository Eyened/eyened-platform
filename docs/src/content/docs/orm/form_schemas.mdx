...
| `form_entity_scope` | Attachment scope for form lookup and creation. One of: `Patient`, `Study`, `Eye`, `StudyEye`, `ImageInstance` (matches `FormSchema.EntityType`). When omitted, the schema's `EntityType` is used. |
| `form_image_scope` | **Deprecated.** When `true`, equivalent to `form_entity_scope: "ImageInstance"`. |
