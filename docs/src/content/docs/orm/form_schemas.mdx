...
When present, SchemaForm shows a **PointField** with an Activate control that arms a shared viewer `PointTool` (place with LMB, delete with RMB). Cardinality comes from ordinary JSON Schema:

| Schema shape | Meaning |
|--------------|---------|
| `type: "object"` with `x` / `y` | Single landmark |
| `type: "array"` of point objects | List of landmarks |
| `additionalProperties` → point or array of points | Per-image map keyed by image **PublicID** (non-`ImageInstance` entity types) |

Optional `"x-eyened-point-mode": "registration"` enables null mid-list slots and numbered focus (used by `Pointset registration`). Validators ignore `x-eyened-*` keys.
...
Form **Activate** publishes an armed field on a shared bridge (`pointArming.armForm`). Each open **MainViewer** mounts its own `PointTool` against that target, so placement works on every main panel and follows add/replace/close without the form owning overlays.
