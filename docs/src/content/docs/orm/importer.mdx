...
So they land in one `Series` for the viewer and for export localizer heuristics. Rows with an explicit `series_id` are left alone. Confirmed links still apply if `series_anonymous_identity` is set (otherwise distinct DICOM series UIDs would keep the pair apart). Modality is **not** written to the row unless `infer_metadata_from_dicom_header=True` (or you set it yourself).
