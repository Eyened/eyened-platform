...
The core data model follows a hierarchical structure: **Project → Patient → Study → Series → ImageInstance → ImageStorage**.
...
- `ThumbnailPath`: Thumbnail path prefix (see below)
- Relationships to `DeviceInstance`, `SourceInfo`, `ModalityTable`, `Scan`, and `ImageStorage`

**File location:** each image has one or more `ImageStorage` rows; the primary row (`IsPrimary`) points to a `StorageBackend` (nginx `Key`) and an `ObjectKey` (path within that backend). Local ORM/worker access resolves paths via `EYENED_STORAGE_MOUNTS`; the API serves files with `GET /api/images/{public_id}/data`.

`DatasetIdentifier` on `ImageInstance` is deprecated — use `ImageStorage` instead.

#### ThumbnailPath

The `ThumbnailPath` column stores the thumbnail path prefix and generation state:

- **NULL**: No thumbnail yet — run a thumbnail job (CLI, API, or `PostImport`)
- **Empty string (`""`)**: Generation was attempted but failed
- **Path prefix**: Success — relative path under `{EYENED_STORAGE_ROOT}/thumbnails/` (no file extension)

The generator writes **144px** and **540px** square JPEGs as `{ThumbnailPath}_144.jpg` and `{ThumbnailPath}_540.jpg`. New prefixes are allocated as `{project_id}/{bucket}/{uuid}` (for example `42/a1/f3e2d1c0b4a5…`).

On disk (local storage):

```
{EYENED_STORAGE_ROOT}/thumbnails/{ThumbnailPath}_144.jpg
```
...
filename = image.get_thumbnail_filename(144)  # e.g. "42/a1/uuid_144.jpg"
pil_image = image.get_thumbnail(144)  # loads bytes via the data access adapter
```

See **[Thumbnails](/eyened-platform/thumbnails)** for generation (CLI, importer, API, worker).
...
Builtin viewer schemas (ETDRS grid, registration) are seeded with `eorm seed-form-schemas`. See **[Form Schemas](/eyened-platform/orm/form_schemas)** for the full list, seeding workflow, and TaskConfig conventions.

:::note
**Fovea coordinates:** AI-detected landmarks live in `ImageInstance.CFKeypoints` (`fovea_xy` tuples). Manually placed ETDRS grids are stored in `FormAnnotation.FormData` (`fovea` / `disc_edge` objects). See **[ETDRS Grid Panel](/eyened-platform/client/etdrs_panel)**.
:::
...
- `TaskConfig`: JSON configuration for the task (for example `form_schema_name` and `form_image_scope` for grading workflows — see **[Form Schemas](/eyened-platform/orm/form_schemas#taskconfig-for-grading-tasks)**)
