...
The import process follows a hierarchical structure where images are organized within projects, patients, studies, and series. When importing an image, you can specify identifiers for each level of this hierarchy. The API will either use existing database entities or create new ones based on the configuration options you provide.
...
:::note
For detailed information about entity fields available for each level (Patient, Study, Series, Image), see the [Entity Fields documentation](entities).
:::

### Single Image Import

The API provides an endpoint for importing individual images:
...
```python
{
  "data": {
    ...
    "image": "/path/to/image.jpg",     # Absolute path to image
    "image_props": {                    # Optional image properties
      ...
      # Any other image properties from entities.mdx
    }
  },
  "options": {
...
The `props` dictionaries at each level can include any of the fields documented in the [Entity Fields documentation](entities.mdx). These properties are used to populate the corresponding database objects.

### Generating Thumbnails

After importing images, you can trigger thumbnail generation with:
...
:::note
For detailed information about entity fields that can be included in the `props` dictionaries, see the [Entity Fields documentation](entities).
:::
