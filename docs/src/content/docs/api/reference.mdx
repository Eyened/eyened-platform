...
Queue a background job to generate thumbnails for all images with missing (or optionally failed) `ThumbnailPath`. Returns a `task_id` for status polling.
...
See [Thumbnails](/eyened-platform/thumbnails) for worker setup and status checks.

### POST /api/import/update_thumbnails_for_image_ids

Queue thumbnail generation for a specific list of `ImageInstanceID`s.

**Request body:** `{ "image_ids": [int, ...], "print_errors": bool }`

**Response:** same `TaskResponse` as above.

### GET /api/import/status/\{task_id\}

Poll RQ job status for any import-queue task (thumbnails, CFI models, etc.).
...
Returns `{ "error": "job not found" }` if the ID is unknown.

Full workflow: [Thumbnails — API](/eyened-platform/thumbnails#api).
