...
Features name what is being segmented (layers, lesions, regions). They support hierarchical parent–child relationships for MultiClass and MultiLabel maps.

**Related:** [Segmentations API](/eyened-platform/api/segmentations) · [Segmentation panel](/eyened-platform/client/panels/segmentation) · [Data model](/eyened-platform/orm/data_model/segmentations).

## API Endpoints

### GET /api/features

- Returns every feature, with its subfeature list
- `?with_counts=true` adds a per-feature segmentation count
