...
**This is not a data migration.** The new stack starts with an empty database, and following this
section does not bring existing data across; moving an existing database into `deploy/` is not
documented here yet, so if you have data you need, do not upgrade yet.
...
- **Breaking: environment variables renamed.** `DEV_NGINX_PORT` → `HTTP_PORT`, the `docker/`
  ...
  One name survived with a new meaning: `EYENED_DATABASE_PORT` used to publish MySQL on the host,
  and now tells the server which port to connect to. A carried-over non-default value points the
  server at a port nothing is listening on.
