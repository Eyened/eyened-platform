...
**This is not a data migration.** The bundled database starts empty, and following this section
does not bring existing data across; moving an existing database into `deploy/` is not documented
here yet, so if you have data you need, do not upgrade yet. A site running against an external
database is the exception — it keeps using that database.
