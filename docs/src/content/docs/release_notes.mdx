...
- **Before you pull: take a database backup with the tooling you have now, and keep the old
  ...
  with that checkout's `database/load_dump.sh`. `./eyened restore` reads only backups
  `./eyened backup` wrote.
...
- **One Docker stack.** `dev/`, `docker/` and `database/` are replaced by a single `deploy/`
  directory driven by one script at the repository root: `./eyened install` for a client install,
  `./eyened up` for a developer stack, `./eyened prod` for a site deployment against an external
  database. Docker is the only prerequisite. The database is part of the stack rather than a
  second compose project to start first. Full reference: `deploy/README.md`.
- **One env file.** `deploy/.env`, generated from `deploy/.env.example` on first run and written
  once — nothing rewrites it afterwards. `dev/.env`, `docker/.env`, `database/.env` and
  `worker/.env` are not read.
...
- **Breaking: `EYENED_STORAGE_ROOT` is no longer an operator setting.** The container path is
  ...
  image datasets are configured in `deploy/storage-mounts.conf`. `EYENED_STORAGE_ROOT` in
  `deploy/.env` is a preflight failure rather than a warning, so carrying that line over from
  `docker/.env` stops the build until you remove it.
- **Breaking: the `Makefile` is gone.** Its three targets — `gen-openapi`, `gen-types`,
  `gen-client-types` — are now the single command `npm run gen:types` in `client/`. `./eyened`
  absorbs none of them: it drives the stack, which the Makefile never did. `make` is not a
  prerequisite.
- **Breaking: Docker Compose 2.26 or newer is now required.** The server's dependency on the
  ...
  dependency, which makes the server race MySQL and crash-loop on first boot. `./eyened doctor`
...
- **Adminer is gone.** It used to start automatically with the `database` stack, on
  `EYENED_ADMINER_PORT` (default `8080`); nothing in `deploy/` starts it. Use `./eyened db-shell`
  for a MySQL shell. For a GUI, append `:compose.host-ports.yaml` to `COMPOSE_FILE` in
  `deploy/.env` to publish MySQL on a loopback host port, then point a locally installed client
  (DBeaver, TablePlus, etc.) at it — see `deploy/README.md`'s **Sharing a machine** section.
...
- **`database/save_dump.sh` and `load_dump.sh` are gone.** `./eyened backup <dir>` and
  `./eyened restore <dir>` are this stack's backup pair, and `restore` reads only what `backup`
  wrote. `backup` never stops the database; `restore` does. For any other database, including an
  external one, `eorm save-dump` / `eorm load-dump` are unchanged. See `deploy/README.md`'s
  **Backup and rollback** section.
- **`./eyened backup` needs four privileges granted by hand, once per install.** The bundled
  database's application account is created by the MySQL image with `ALL PRIVILEGES` on its own
  schema and nothing else, which XtraBackup is refused on — `./eyened backup` exits 1 with
  `LOCK INSTANCE FOR BACKUP ... you need (at least one of) the BACKUP_ADMIN privilege(s)`. This
  affects every bundled-database install, a fresh one included, not just a database brought across
  from the old stack. The `GRANT` statement, and what it means for the account the application connects as, are
  in `deploy/README.md` under **Moving an existing database into this stack**.
...
- **After moving the checkout, rebuild.** Images are built from your checkout, so a plain
  `docker compose up -d` reuses the images you already have and picks up nothing. `./eyened
  install` and `./eyened up` rebuild every time; by hand it is `docker compose up -d --build`.
...
- **Storage mounts have one source of truth.** `deploy/storage-mounts.conf` generates the
  container bind mounts, the nginx `location` blocks, and the server container's
  `EYENED_STORAGE_MOUNTS` — three things that previously had to be edited by hand, in agreement,
  across the compose file, `nginx.conf` and the env file. Getting one of the three wrong is what
  made Python-side pixel reads (thumbnail regeneration, inference, reports) fail while the viewer
  still worked.
- **No placeholder secrets.** `docker/.env.example` and `database/.env.example` shipped
  ...
  none: first run generates them into `deploy/.env`, and a hand-copied `.env` fails at `docker
  compose up` until real values are set. That is deliberate, not a regression.
