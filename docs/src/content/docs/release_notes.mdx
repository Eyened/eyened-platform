...
- **`./eyened backup` needs four privileges granted by hand, once per install.** The bundled
  ...
  affects every install, a fresh one included, not just a database brought across from the old
  stack. The `GRANT` statement, and what it means for the account the application connects as, are
