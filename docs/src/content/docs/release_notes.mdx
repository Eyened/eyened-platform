...
- **`./eyened backup` needs four privileges granted by hand, once per install.** The bundled
  ...
  from the old stack. The `GRANT` statement, and what it means for the account the application
  connects as, are in `deploy/README.md` under **Moving an existing database into this stack**.
