...
- **Storage mounts have one source of truth.** `deploy/storage-mounts.conf` generates the
  ...
  still worked.
- **No placeholder secrets.** `dev/sample.env` and `docker/.env.example` shipped `change_me`
  values that kept working if you never changed them. `deploy/.env.example` ships none: first run
  generates them into `deploy/.env`, and a hand-copied `.env` fails at `docker compose up` until
  real values are set. That is deliberate, not a regression.
