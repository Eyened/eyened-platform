...
- **Upgrading an existing stack: rebuild once.** Routine `docker compose up -d` reuses the
  ...
  `docker compose up -d --build` (or re-run `./eyened install` — `./eyened up` on a developer
  stack — which do it) one time.
  ...
  afterwards.
- **Keycloak is part of the stack.** `dev/keycloak/` moved to
  `deploy/keycloak/` and the standalone `keycloak/docker-compose.keycloak.yml` overlay is gone.
  OIDC setup used to mean adding `oidc` to `COMPOSE_PROFILES` in `deploy/.env` and setting
  `PUBLIC_HOST`, with no second compose file to layer; it now means appending
  `:compose.oidc.yaml` to `COMPOSE_FILE` instead, and an old `COMPOSE_PROFILES=oidc` is
  silently inert rather than an error. Existing OIDC setups must also rename
  `DEV_KEYCLOAK_PORT` to `KEYCLOAK_PORT` — see the renames below — and set the
...
- **One Docker stack.** `dev/`, `docker/`, and `database/` are replaced by a single `deploy/`
  directory. Clients install with `./eyened install` (Docker is the only prerequisite);
  developers use `./eyened up`; site deployments against an external database use
  `./eyened prod`. The database is part of the stack rather than a second compose project.
- **Upgrading: bring the old stacks down before you pull.** Deleting those directories takes their
  compose files with them, so afterwards `cd dev && docker compose down` fails with "no
  configuration file" and the legacy volumes — `db_data` from the `database` stack,
  `client_node_modules` from the `dev` one — have nothing left to remove them. Nothing in `deploy/`
  touches either. If you have already pulled, Compose can still address a project by name with no
  file present (`docker compose -p <project> ps` lists it), or recover the file itself with
  `git show <ref-before-this-release>:dev/docker-compose.yml`.
...
- **Breaking: `make` and `./install.sh` are gone.** Both were replaced by one script,
  `./eyened`, which absorbs every `make` target except `gen-openapi`, `gen-types`, and
  `gen-client-types` — those three move to `npm run gen:types` in `client/`. `make` is no
  longer a prerequisite, and there is no Makefile. An operator upgrading through this whole
  cycle meets every item below at once.
- **`oidc` is a compose layer, not a profile.** Append `:compose.oidc.yaml` to `COMPOSE_FILE`
  in `deploy/.env` instead of adding `oidc` to `COMPOSE_PROFILES` — see the Keycloak bullet
  above, which described the profile-based form this replaces.
- **The `tools` profile and Adminer are gone.** Use `./eyened db-shell` for a MySQL shell. For
  a GUI, append `:compose.host-ports.yaml` to `COMPOSE_FILE` in `deploy/.env` to publish MySQL
  on a loopback host port, then point a locally installed client (DBeaver, TablePlus, etc.) at
  it — see `deploy/README.md`'s **Sharing a machine** section.
- **`make db-snapshot` / `db-restore` are gone.** `./eyened backup` and `./eyened restore`
  replace them and are not a rename: the new pair is xtrabackup-based, takes a path rather
  than `NAME=`, and never stops the database to take a backup — only `restore` does. See
  `deploy/README.md`'s **Backup and rollback** section. Anyone with existing archives under the
  old default path, `deploy/snapshots/`, should move or delete them — `.gitignore` covers that
  path again, but they are not moved automatically, and `./eyened backup` has no default
  destination: `deploy/backups/` is merely the suggested one (`<dir>` is a required argument).
- **`worker/`'s compose files and `.env` are gone.** The four `worker/docker-compose.*.yml`
  files and `worker/.env*` are replaced by `deploy/compose.workers.yaml`, which reads
  `deploy/.env` — there is no separate worker env file any more. See `worker/README.md`.
- **`deploy/.env.example` no longer ships placeholder secrets.** A hand-copied `.env` now fails
  at `docker compose up` until real values are set — that is deliberate, not a regression.
- **An existing `deploy/.env` keeps working.** Nothing rewrites it, so a `COMPOSE_PROFILES`
  still naming `oidc` or `tools` is inert rather than an error — it is simply ignored. For
  `oidc`, add `:compose.oidc.yaml` to `COMPOSE_FILE` above to get the same behavior back. There
  is no replacement layer for `tools` — use `./eyened db-shell` instead, per the bullet above.
