...
The Eyened database follows a hierarchical structure where images are categorized into projects, patients, studies, and series (top to bottom). When importing an image, you specify identifiers for each level in a flat `data` object. The API will either use existing database objects or create new ones based on the `options` you provide.
...
This endpoint accepts a JSON payload with a flat `data` object:
...
- `modality` (str) — Image modality (for example `"ColorFundus"`, `"OCT"`)

**Optional hierarchy fields:** `patient_identifier`, `sex`, `birth_date`, `study_date`, `study_description`, `series_id`, `series_number`, `series_instance_uid`. Additional image metadata fields (for example `laterality`, `width`, `height`, `resolution_horizontal`) are documented in [Metadata Fields](/eyened-platform/import_metadata_fields).
...
For bulk imports, patching existing metadata, and repeatable “plan → review → apply” workflows, use the renewed ORM importer based on **flat rows** (`ImportRow`) and an **import run** (`ImportRun`).

The workflow is:

- Build `ImportRow` objects (or load them from CSV)
- `plan_image_import(session, rows, ...)` → returns an `ImportRun` (image rows; merges defaults / inference then plans)
- Review with `run.display_summary()` / `run.full_summary()`
- Apply with `run.apply()` and `session.commit()`
- Optionally write/read JSON for audit + undo (`run.write_json(...)` / `ImportRun.read_json(...)`)

See **[ORM Importer](/eyened-platform/orm/importer)** for the up-to-date guide and examples.
