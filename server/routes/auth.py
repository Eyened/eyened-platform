...
def is_admin_user(current_user: CurrentUser, session: Session) -> bool:
    """Return True when the authenticated user has admin privileges."""
    creator = current_user.get_creator(session)
    if creator is None:
        return False

    role = creator.Role
    if role in (1, "1", "admin", "Admin"):
        return True

    admin_username = getattr(settings, "admin_username", None)
    return bool(admin_username and creator.CreatorName == admin_username)


def require_admin(current_user: CurrentUser, session: Session) -> None:
    """Raise 403 if the current user is not an admin."""
    if not is_admin_user(current_user, session):
